...
import hashlib,hmac,secrets,sqlite3,threading
from dataclasses import dataclass
from datetime import datetime,timezone
from .storage import utcnow
PERMISSIONS={"viewer":{"read"},"operator":{"read","measure","work_order","allocate"},"engineer":{"read","measure","work_order","allocate","analyze"},"quality":{"read","measure","work_order","allocate","analyze","approve"},"admin":{"read","measure","work_order","allocate","analyze","approve","admin"}}
...
class Auth:
    ...
    def create_user(self,user_id,password,role="operator"):
        ...
        with self._lock: self.db.execute("INSERT INTO users VALUES(?,?,?,?,1,?)",(user_id,role,salt,_digest(password,salt),utcnow())); self.db.commit()
    ...
    def deactivate(self,user_id):
        with self._lock: self.db.execute("UPDATE users SET active=0 WHERE user_id=?",(user_id,)); self.db.execute("UPDATE sessions SET active=0 WHERE user_id=?",(user_id,)); self.db.commit()
